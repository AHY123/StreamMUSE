...
def continuation(model, midi_path, prompt_length=100, generation_length=384, temperature=1.0, n_samples=1, gt_mel=True):
    ...
    if not gt_mel:
        decode_output(
            ...
            f"toAndy/{model.save_name}/{os.path.basename(midi_path)}_originalmelody.mid",
            single=True,
            tempo=90.0,
        )
    ...
    if prompt_length != 0:
        decode_output(
            [x[:, i, :] for i in range(x.shape[1])], f"toAndy/{model.save_name}/{os.path.basename(midi_path)}_promptlen{prompt_length}.mid", tempo=90.0
        )
    ...
    for i in range(n_samples):
        ...
        decode_output(output_i, f"toAndy/{model.save_name}/prompt{prompt_length}/{os.path.basename(midi_path)}_temp{temperature}_{i}.mid", tempo=90.0)
...
if __name__ == "__main__":
    ...
    model_schema = NewM2ATransformerSchema(hidden_size=768, num_layers=12, num_attention_heads=8, intermediate_size=2048,local_model_num_attention_heads=8,local_model_num_layers=6, local_model_intermediate_size=2048)
    if "small" in model_path:
        model = NewM2ATransformer.load_from_checkpoint(model_path,model_schema=model_schema)
    else:
        model = NewM2ATransformer.load_from_checkpoint(model_path,model_schema=model_schema)
