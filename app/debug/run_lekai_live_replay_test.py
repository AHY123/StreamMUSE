...
from input_handlers.input_handler import KEY_TO_PITCH
from lekai_input_debug import CLIENT_TIMING_DEBUG_PATH
from midi_utils_client import midi_to_note_client
...
OUTPUT_DIR = ROOT / "app" / "debug" / "live_replay_output"
CLIENT_TIMING_LOG = Path(CLIENT_TIMING_DEBUG_PATH)

PITCH_TO_KEY = {pitch: key for key, pitch in KEY_TO_PITCH.items()}
...
def replay_keyboard(notes: list[dict], tempo: float, ticks_per_beat: int, start_delay: float):
    ...
    note_events = []
    for note in notes:
        key = PITCH_TO_KEY[int(note["pitch"])]
        note_events.append((note["tick"], "press", key))
        note_events.append((note["tick"] + note["duration"], "release", key))
    note_events.sort(key=lambda item: (item[0], 0 if item[1] == "release" else 1))

    start_time = time.perf_counter() + start_delay
    for tick, action, key in note_events:
...
def run_case(case_name: str, notes: list[dict], tempo: float, ticks_per_beat: int):
    ...
    try:
        time.sleep(2.0)
        replay_keyboard(notes, tempo, ticks_per_beat, start_delay=1.0)
...
def main():
    parser = argparse.ArgumentParser(description="Run Lekai live keyboard replay tests.")
    parser.add_argument(
        "--case",
