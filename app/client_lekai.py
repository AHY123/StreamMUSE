...
def _debug_log_user_event(event: dict):
    """Append user note events to a temp file (JSONL).

    Path: /tmp/streammuse_user_events.jsonl (macOS/Linux)
    """
    try:
        et = event.get("type")
        ...
        with open(_USER_EVENT_DEBUG_PATH, "a", encoding="utf-8") as f:
    ...
    while True:
        ...
        while not event_queue.empty():
            ...
            if event["type"] == "note_on":
                ...
                # Pass the note_off event directly to the audio handle
